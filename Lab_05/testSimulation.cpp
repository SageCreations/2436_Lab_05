...
void generateStatistics(serverListType& serverList,
                        waitingCustomerQueueType& CQueue, int numOfCustArrived,
...
void runSimulation() {
    ...
    setSimulationParameters(simulationTime, numberOfServers, transactionTime,
    ...
    cout << "The simulation ran for " << simulationTime << " time units"
         << endl;
    ...
    generateStatistics(serverList, customerQueue, custNumber,
                       totalWaitTimeServedCustomers);
}
...
void generateStatistics(serverListType& serverList,
    ...
    int numberOfCustomersServed =
        numOfCustArrived - customersLeftInServers - customersLeftInQueue;
    ...
    if (numOfCustArrived > 0)  // If number of customers arrived is > 0
        averageWaitTime =
            (static_cast<double>(totalWaitTime)) / numOfCustArrived;
